...
void RegionTester::testCreateFromStream()
{
    ...
    // Load a file with the and four nations
    {
        ...
        std::ifstream inputStream(inputFile);
        std::shared_ptr<Region> world = Region::create(inputStream);
        if (world==nullptr)
        {
            std::cout << "Failed to create a region from " << inputFile << std::endl;
            return;
        ...
        {
            std::shared_ptr<Region> nation = world->getSubRegionByIndex(nationIndex);
...
void RegionTester::testCreateFromString()
{
...

    {
        ...
        std::shared_ptr<Region> region = Region::create(inputString);
        if (region == nullptr) {
...
void RegionTester::testCreateFromTypeAndString()
{
    ...
    {
        std::string inputString = "Tiny Country,30,20";
        std::shared_ptr<Region> region = Region::create(Region::NationType, inputString);
        if (region == nullptr) {
            std::cout << "Failed to create a region from " << inputString << std::endl;
            return;
        }
...

    {
        ...
        std::shared_ptr<Region> region = Region::create(Region::RegionType::StateType, inputString);
        if (region == nullptr) {
            std::cout << "Failed to create a region from " << inputString << std::endl;
            return;
        }
...

    {
        ...
        std::shared_ptr<Region> region = Region::create(Region::RegionType::CountyType, inputString);
        if (region == nullptr) {
            std::cout << "Failed to create a region from " << inputString << std::endl;
            return;
        }
...

    {
        ...
        std::shared_ptr<Region> region = Region::create(Region::RegionType::CityType, inputString);
        if (region == nullptr) {
            std::cout << "Failed to create a region from " << inputString << std::endl;
            return;
        }
...

    {
        ...
        std::shared_ptr<Region> r1 = Region::create(Region::NationType, inputString);
        if (r1 != nullptr) {
            std::cout << "Failed to recognize a bad data string in create method, inputString=" << inputString << std::endl;
            return;
        }
...
void RegionTester::testSubRegions()
{
    ...
    std::ifstream inputStream(inputFile);
    std::shared_ptr<Region> region = Region::create(inputStream);
    for(auto i = 0; i < region->getSubRegionSize()-1; i++) {
        if(!region->getIsValid()) {
            std::cout << "Sub region not valid.\n";
            return;
        }
        if(region->subRegionExists(i)) {
            if(!region->getSubRegionByIndex(i)->getIsValid()) {
                std::cout << "Sub region not valid.\n";
                return;
            }
        }
...
void RegionTester::testComputeTotalPopulation()
{
    ...
    if(region->computeTotalPopulation() != 4059243) {
        std::cout << "\tTest failed; got \"" << region->getTotalPopAtLevel() << "\" instead of " << 4059243 << std::endl;
        return;
    }
}
